...
$(window).load(function(){
	var nav=$('footer nav'),glob_nav=$('#glob>nav'),content=$('#content')
$('.gallery .thumbs').uCarousel({show:3})
$('.gallery .thumbs').navs(function(n,_){$('.gallery .pic').sImg(_.href)}).navs(0)
content.tabs({empty:'#!/home',actFu:function(_){if(_.prev)
_.prev.stop().animate({opacity:0},{duration:600,complete:function(){_.prev.hide().css({opacity:1})}})
if(_.curr)
_.curr.css({left:'100%',marginLeft:'0px'}).show().stop().animate({left:'50%',marginLeft:'-470px'},{duration:900})}})
glob_nav.navs({useHash:true,defHash:'#!/home'})
nav.navs({useHash:true,defHash:'#!/home'}).navs(function(n,_){if(n===_.defHash)
show_splash()
else
show_subpages()
if(n==='#!/contact')
...
setTimeout(function(){glob_nav.addClass('splash')},500)
$('footer').fadeIn('slow');$('.offer').hide();$('.zakajnas').show();$('.offer_nav').animate({top:'-50px'});$('body').animate({top:'+=170px'})}
function show_subpages(){if(show_subpages.ready)
return false
show_subpages.ready=true
glob_nav.removeClass('splash')
setTimeout(function(){glob_nav.addClass('subpages')},500)
$('footer').fadeOut('fast');$('.zakajnas').hide();$('.offer_nav').animate({top:'+50px'});$('body').animate({top:'-=170px'})}
...
holder.html(str)}
$('#gspinner').fadeOut(function(){$('body').css({overflow:'auto'})})});
(function($){var c=['DOMMouseScroll','mousewheel'];
$.event.special.mousewheel={setup:function(){if(this.addEventListener)for(var i=c.length;i;)this.addEventListener(c[--i],handler,false);
else this.onmousewheel=handler},teardown:function(){if(this.removeEventListener)for(var i=c.length;i;)this.removeEventListener(c[--i],handler,false);
	else this.onmousewheel=null}};
$.fn.extend({mousewheel:function(a){return a?this.bind("mousewheel",a):this.trigger("mousewheel")},unmousewheel:function(a){return this.unbind("mousewheel",a)}});
function handler(a){var b=[].slice.call(arguments,1),delta=0,returnValue=true;
	...
	return $.event.handle.apply(this,b)}})(jQuery);
